...
# 1. We need to find the summarizer Right View block which is currently inserted right after `{/* RIGHT COLUMN: Output */}`
...
# Let's completely recreate App.jsx from retheme.py, and insert things carefully at the exact right spots.
...
"""
content = content.replace("{/* LEFT COLUMN: Input */}\n        <div className=\"w-full lg:w-[45%] xl:w-[40%] h-full flex flex-col bg-white border-r border-zinc-200/80 z-10 shadow-[4px_0_24px_rgba(0,0,0,0.02)] hide-scroll overflow-y-auto\">", "{/* LEFT COLUMN: Input */}\n        <div className=\"w-full lg:w-[45%] xl:w-[40%] h-full flex flex-col bg-white border-r border-zinc-200/80 z-10 shadow-[4px_0_24px_rgba(0,0,0,0.02)] hide-scroll overflow-y-auto\">\n" + left_view)
...
"""
noise_bg = "<div className=\"absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-[0.03] pointer-events-none mix-blend-overlay\"></div>"
content = content.replace(noise_bg, noise_bg + "\n" + right_view)

with open(r'c:\oosc\Judge\rti-frontend\src\App.jsx', 'w', encoding='utf-8') as f:
    f.write(content)
print("Frontend layout completely fixed.")
